...
class Trans {
    ...
    template <typename A,
              enable_if_t<is_same_v<A, CImgList<typename A::value_type>>,
                          bool> = true>
   ...
   public:
     ...
     */
    template <typename A>
    static inline untranslated<A> untranslate(A a, registry *reg = nullptr)
    {
        if constexpr (is_untranslatable<A>::value) {
            ...
            if (reg) {
                ...
                if (it != reg->end()) {
                    ...
                    return *static_cast<decltype(unsubstitute<A>(
                        declval<A>())) *>(b);
                }
            }
