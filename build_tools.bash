...
function 33_build_manylinux () {
    ...
    docker run --rm -e PLAT=$PLAT -e PYBIN_PREFIX=$PYBIN_PREFIX -v `pwd`:/io $DOCKER_IMAGE $PRE_CMD /bin/bash /io/manylinux/test-wheels.sh || { echo "Many linux test wheels script failed. Exiting" ; exit 1; }
    ls -lhart wheelhouse/
    set +x
}
